...
**Project overview**
This repository contains a face-recognition pipeline prepared for deployment on NVIDIA Jetson devices 
...
> Warning: TensorFlow builds and versions on Jetson devices vary by JetPack version. See NVIDIA's official JetPack and TensorFlow installation guides for the Jetson platform. 
...
- Flash your Jetson with an appropriate JetPack (Jetson Linux) image using SDK Manager. JetPack provides the GPU drivers, CUDA, cuDNN and TensorRT required for acceleration. 
- Install a Jetson-compatible TensorFlow package (NVIDIA provides pip wheels and install instructions for supported JetPack releases). Check the official guide for instructions and compatible TF versions for your JetPack. 
...
```
**Jetson note:** On Jetson, install the NVIDIA-provided TensorFlow wheel for the correct JetPack / Python combination (see NVIDIA docs). Attempting to `pip install tensorflow` blindly may not provide GPU support on Jetson. 
